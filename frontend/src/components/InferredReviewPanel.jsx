    ...
    <div className="card" style={{ padding: 16 }}>
      <div className="row" style={{ justifyContent: 'space-between', marginBottom: 12 }}>
        ...
        <div className="row" style={{ gap: 6 }}>
          {['pending_review', 'ready_to_build', 'promoted', 'rejected', 'all'].map(s => (
            <button
              key={s}
            ...
            >
              {{ pending_review: '待审', ready_to_build: 'Build 队列', promoted: '已入库', rejected: '已拒绝', all: '全部' }[s]}
            </button>
          ))}
      ...
      {inferred.length > 0 && inferredFilter === 'pending_review' && (
      ...
      {inferred.length === 0 ? (
        <div className="muted" style={{ padding: 28, textAlign: 'center' }}>
          暂无{ { pending_review: '待审', ready_to_build: 'Build 队列内', promoted: '已入库', rejected: '已拒绝', all: '' }[inferredFilter] }候选。先到「文件夹 / 历史用例」运行五阶段分析。
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {getCurrentPageItems().map(it => {
            const isReadyAuto = it.review_status === 'ready_to_build' && it.auto_accepted;
            const isReadyManual = it.review_status === 'ready_to_build' && !it.auto_accepted;
            const isPromoted = it.review_status === 'promoted';
            return (
            <div key={it.inferred_id} style={{
              border: `1px solid ${isReadyAuto ? 'rgba(127,217,168,0.35)' : isPromoted ? 'rgba(207,188,255,0.35)' : '#2b292f'}`,
              borderRadius: 8, padding: 12,
              background: isReadyAuto ? 'rgba(127,217,168,0.06)' : isPromoted ? 'rgba(207,188,255,0.06)' : 'rgba(15,13,19,0.3)',
            }}>
              <div className="row" style={{ justifyContent: 'space-between', marginBottom: 6 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                  ...
                  {isReadyAuto && <span className="tag" style={{ background: 'rgba(127,217,168,0.18)', color: '#7fd9a8' }}><span className="mi" style={{ fontSize: 12, verticalAlign: -2, marginRight: 2 }}>auto_awesome</span>AI自动通过 · 待 Build</span>}
                  {isReadyManual && <span className="tag ok">已通过 · 待 Build</span>}
                  {isPromoted && <span className="tag" style={{ background: 'rgba(207,188,255,0.18)', color: '#cfbcff' }}><span className="mi" style={{ fontSize: 12, verticalAlign: -2, marginRight: 2 }}>verified</span>已入库</span>}
                  {it.review_status === 'rejected' && <span className="tag err">已拒绝</span>}
                  {it.review_status === 'pending_review' && <span className="tag warn">待审</span>}
              ...
              <div className="row" style={{ gap: 6, marginTop: 10, justifyContent: 'flex-end' }}>
                ...
                {isReadyManual && editingInferredId !== it.inferred_id && (
                  <button className="ghost" style={{ padding: '4px 12px', fontSize: 12 }} onClick={() => startEditInferred(it)}>
                    <span className="mi" style={{ fontSize: 14, verticalAlign: -2, marginRight: 2 }}>edit</span>
                    修改
                  </button>
