...
class ProjectManager:
    ...
    def batch_root(self, name: str, kind: str) -> Path:
        """Root dir for batch generation folders. kind ∈ {testcase, xmind}."""
        sub = "testcases" if kind == "testcase" else "xmind"
        d = self.root_out / sub / sanitize_name(name)
        d.mkdir(parents=True, exist_ok=True)
        return d

    def batch_dir(self, name: str, kind: str, batch_name: str) -> Path:
        """A specific batch's folder."""
        d = self.batch_root(name, kind) / batch_name
        d.mkdir(parents=True, exist_ok=True)
        return d
