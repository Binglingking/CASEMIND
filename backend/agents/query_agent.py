...
from backend.core.llm import LLMConfig, chat, try_parse_json
...
QueryMode = str  # "qa" | "testcase" | "xmind"
...
class QueryAgent(AgentBase):
    ...
    def run(self, question: str, mode: QueryMode, llm_cfg: LLMConfig,
            ...
            reference_blocks: list[dict] | None = None) -> dict:
        hist = _compact_history(history or [])
        ref_text = _format_references(reference_blocks or [])

        if mode == "chat":
            return self._chat(question, llm_cfg, hist, ref_text)
        ...
        if mode == "testcase":
            return self._testcase(question, ctx, memory_prompt, retrieved, llm_cfg, hist, ref_text)
        if mode == "xmind":
            return self._xmind(question, ctx, memory_prompt, retrieved, llm_cfg, hist, ref_text)
        return self._qa(question, ctx, memory_prompt, retrieved, llm_cfg, hist, ref_text)
    ...
    def _chat(self, q, llm_cfg, hist, ref_text=""):
        sys_prompt = (
        ...
        raw = chat(
            messages=self._build_messages(sys_prompt, q, hist, ref_text),
    ...
    def _qa(self, q, ctx, memory_prompt, retrieved, llm_cfg, hist, ref_text=""):
        ...
        raw = chat(
            messages=self._build_messages(sys_prompt, user, hist, ref_text),
    ...
    def _testcase(self, q, ctx, memory_prompt, retrieved, llm_cfg, hist, ref_text=""):
    ...
    def _xmind(self, q, ctx, memory_prompt, retrieved, llm_cfg, hist, ref_text=""):
