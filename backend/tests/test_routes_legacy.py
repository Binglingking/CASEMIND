...
def _seed_inferred(review_status: str = "pending_review", *, auto_accepted: bool = False):
    from backend.schemas.inferred_kp import InferredKnowledgePoint, InferredSource
    item = InferredKnowledgePoint(
        inferred_id="IKP_seed0001",
        type="business_rule",
        content="登录成功跳转首页",
        module="账号",
        source=InferredSource(
            kind="case", file="cases.xlsx", file_id="seed",
            case_id="LC_seed_0002", case_row=2,
        ),
        confidence=0.95,
    ...
    legacy_store.save_inferred_kps(PROJECT, [item])
    return item


def test_inferred_review_accept_goes_to_ready_to_build(client):
    _seed_inferred("pending_review")
    r = client.post("/api/legacy/inferred/review", json={
        "project": PROJECT, "inferred_id": "IKP_seed0001", "decision": "accept",
    })
    assert r.status_code == 200
    assert r.json()["item"]["review_status"] == "ready_to_build"


def test_inferred_review_reject(client):
    _seed_inferred("pending_review")
    r = client.post("/api/legacy/inferred/review", json={
        "project": PROJECT, "inferred_id": "IKP_seed0001", "decision": "reject",
    })
    assert r.status_code == 200
    assert r.json()["item"]["review_status"] == "rejected"


def test_inferred_revoke_only_ready_auto(client):
    _seed_inferred("ready_to_build", auto_accepted=True)
    r = client.post("/api/legacy/inferred/revoke", json={
        "project": PROJECT, "inferred_id": "IKP_seed0001", "decision": "revoke",
    })
    assert r.status_code == 200
    assert r.json()["item"]["review_status"] == "pending_review"
    assert r.json()["item"]["auto_accepted"] is False


def test_inferred_revoke_rejects_manual_ready(client):
    _seed_inferred("ready_to_build", auto_accepted=False)
    r = client.post("/api/legacy/inferred/revoke", json={
        "project": PROJECT, "inferred_id": "IKP_seed0001", "decision": "revoke",
    })
    assert r.status_code == 400
...
def test_inferred_promote_writes_kp_and_marks_promoted(client):
    _seed_case_file()
    _seed_inferred("ready_to_build", auto_accepted=True)
    r = client.post("/api/legacy/inferred/promote", params={"project": PROJECT})
    assert r.status_code == 200
    body = r.json()
    assert body["ok"] is True
    assert body["promoted_count"] == 1
    assert len(body["kp_ids"]) == 1

    # 候选状态切到 promoted 且写入 promoted_kp_id
    on_disk = legacy_store.load_inferred_kps(PROJECT)
    assert on_disk[0].review_status == "promoted"
    assert on_disk[0].promoted_kp_id == body["kp_ids"][0]

    # 再次调用幂等：promoted_count=0，skipped_already_promoted=1
    r2 = client.post("/api/legacy/inferred/promote", params={"project": PROJECT})
    assert r2.json()["promoted_count"] == 0
    assert r2.json()["skipped_already_promoted"] == 1


def test_inferred_stats_endpoint(client):
    _seed_inferred("ready_to_build", auto_accepted=True)
    r = client.get("/api/legacy/inferred/stats", params={"project": PROJECT})
    assert r.status_code == 200
    body = r.json()
    assert body["ready_to_build_count"] == 1
    assert body["pending_review_count"] == 0
    assert body["promoted_count"] == 0


def test_legacy_status_migration_on_load(tmp_settings):
    """旧的 pending/accepted/auto_accepted 在 load 时迁移到新三态。"""
    legacy_dir = legacy_store.legacy_dir(PROJECT)
    legacy_dir.mkdir(parents=True, exist_ok=True)
    raw = [
        {
            "inferred_id": "IKP_old_pending", "type": "business_rule",
            "content": "x", "module": "m",
            "source": {"kind": "case", "file": "f.xlsx", "file_id": "fid"},
            "confidence": 0.5, "extracted_at": utc_iso_z(),
            "review_status": "pending",
        },
        {
            "inferred_id": "IKP_old_accepted", "type": "business_rule",
            "content": "y", "module": "m",
            "source": {"kind": "case", "file": "f.xlsx", "file_id": "fid"},
            "confidence": 0.95, "extracted_at": utc_iso_z(),
            "review_status": "accepted",
        },
        {
            "inferred_id": "IKP_old_promoted", "type": "business_rule",
            "content": "z", "module": "m",
            "source": {"kind": "case", "file": "f.xlsx", "file_id": "fid"},
            "confidence": 0.95, "extracted_at": utc_iso_z(),
            "review_status": "accepted",
            "promoted_kp_id": "KP_xxx",
        },
    ]
    (legacy_dir / "inferred_kps.json").write_text(
        json.dumps(raw, ensure_ascii=False), encoding="utf-8"
    )

    items = {i.inferred_id: i for i in legacy_store.load_inferred_kps(PROJECT)}
    assert items["IKP_old_pending"].review_status == "pending_review"
    assert items["IKP_old_accepted"].review_status == "ready_to_build"
    assert items["IKP_old_promoted"].review_status == "promoted"
